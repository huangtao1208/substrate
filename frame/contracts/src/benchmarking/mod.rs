...
impl<T: Trait> Tombstone<T> {
	...
	fn new(stor_num: u32, stor_size: u32) -> Result<Self, &'static str> {
		...
		System::<T>::set_block_number(
			contract.eviction_at()? + T::SignedClaimHandicap::get() + 5.into()
		);
...
benchmarks! {
	...
	call {
		...
		System::<T>::set_block_number(instance.eviction_at()? - 5.into());
	...
	claim_surcharge {
		...
		System::<T>::set_block_number(
			instance.eviction_at()? + T::SignedClaimHandicap::get() + 5.into()
		);
	...
	}: call(origin, callee, 0.into(), Weight::max_value(), vec![])
	verify {
		for addr in &addresses {
			instance.alive_info()?;
		}
	...
	// w_call = w_bench - 2 * w_param
	// Slow instruction using API repetition parameters (see instr_call).
	instr_call_indirect {
		let r in 0 .. API_BENCHMARK_BATCHES;
	...
	instr_local_set {
		let r in 0 .. INSTR_BENCHMARK_BATCHES;
		use body::DynInstr::{RandomI64, RandomSetLocal};
		let max_locals = Contracts::<T>::current_schedule().limits.stack_height;
		let mut call_body = body::repeated_dyn(r * INSTR_BENCHMARK_BATCH_SIZE, vec![
			RandomI64(i64::min_value(), i64::max_value()),
	...
	instr_local_tee {
		let r in 0 .. INSTR_BENCHMARK_BATCHES;
		use body::DynInstr::{RandomI64, RandomTeeLocal, Regular};
		let max_locals = Contracts::<T>::current_schedule().limits.stack_height;
		let mut call_body = body::repeated_dyn(r * INSTR_BENCHMARK_BATCH_SIZE, vec![
			RandomI64(i64::min_value(), i64::max_value()),
	...
	instr_global_set {
		let r in 0 .. INSTR_BENCHMARK_BATCHES;
		use body::DynInstr::{RandomI64, RandomSetGlobal};
		let max_globals = Contracts::<T>::current_schedule().limits.globals;
		let mut sbox = Sandbox::from(&WasmModule::<T>::from(ModuleDefinition {
			call_body: Some(body::repeated_dyn(r * INSTR_BENCHMARK_BATCH_SIZE, vec![
				RandomI64(i64::min_value(), i64::max_value()),
	...
	instr_memory_grow {
		...
		let repetitions = r * API_BENCHMARK_BATCHES;
		let mut sbox = Sandbox::from(&WasmModule::<T>::from(ModuleDefinition {
